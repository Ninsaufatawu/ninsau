...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement, useWindowEvents: boolean) {
        this.container = container;
        this.useWindow = useWindowEvents;
        const target: EventTarget = useWindowEvents ? window : container;
        target.addEventListener('mousemove', this._onMouseMove as EventListener);
        target.addEventListener('touchstart', this._onTouchStart as EventListener, { passive: true } as AddEventListenerOptions);
        target.addEventListener('touchmove', this._onTouchMove as EventListener, { passive: true } as AddEventListenerOptions);
        if (!useWindowEvents) {
          container.addEventListener('mouseenter', this._onMouseEnter as EventListener);
          container.addEventListener('mouseleave', this._onMouseLeave as EventListener);
          container.addEventListener('touchend', this._onTouchEnd as EventListener);
        }
      }
      dispose() {
        const c = this.container;
        if (!c) return;
        const target: EventTarget = this.useWindow ? window : c;
      ...
      onMouseEnter() {
        if (!this.useWindow) this.isHoverInside = true;
      }
      onMouseLeave() {
        if (!this.useWindow) this.isHoverInside = false;
      }
      update() {
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        Mouse.init(props.$wrapper, !!props.useWindowEvents);
